####Predictive Mean Matching Imputation of Ordinal variables####
...
#Impute the data using MICE package, method=pmm for predictive mean matching
...
*method="pmm"* sets the code for predictive mean matching.
...
```{r}
tempdata.five.1<-mice(ord.five.1, m=10, maxit=50, method="pmm", seed=1)
tempdata.five.2<-mice(ord.five.2, m=10, maxit=50, method="pmm", seed=1)
tempdata.five.3<-mice(ord.five.3, m=10, maxit=50, method="pmm", seed=1)
tempdata.five.4<-mice(ord.five.4, m=10, maxit=50, method="pmm", seed=1)
tempdata.five.5<-mice(ord.five.5, m=10, maxit=50, method="pmm", seed=1)
```
...
write.csv(ab, file="PE.ord.pmm.practice.csv")
```
...
df.ord.pmm<-data.frame(rbind(aa, ab, ac, ad, ae))
colnames(df.ord.pmm)<-c("PH", "CO", "PD", "LEH", "periostosis")
rownames(df.ord.pmm)<-c("five_one_imp", "five_two_imp", "five_three_imp", "five_four_imp", "five_five_imp")
write.csv(df.ord.pmm, file="nrmse.ordinal.pmm.practice.csv")
```