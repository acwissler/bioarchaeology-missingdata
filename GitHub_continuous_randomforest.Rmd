...
*continuousdata.csv* is the complete dataset - i.e. no missing data. The complete dataset is needed in order to evaluate the success of the imputation/deletion method

*cont.five.1* is the first version of the complete dataset that has 5% of the values missing.
*cont.five.2* is the second version of the complete dataset that has 5% of the values missing. Etc. Each of the 5 versions has different values missing.
...
*method="rf"* sets the code for random forest imputation
*seed=1* is the starting number R will start at to run the iterations. If you don't set a seed # you will get a slightly different output each time. 
...
#STEP 4# Pool Dataset
Each of the m=10 versions of the dataset must now be pooled, creating 5 datasets that incorporate the variability in values
...
write.csv(ab, file="PE.cont.reg.csv")
```
...
df.cont.reg<-data.frame(rbind(aa, ab, ac, ad, ae))
colnames(df.cont.reg)<-c("FL.Left", "FL.Right", "T1.AP", "T1.TR", "T5.AP", "T5.TR", "T10.AP", "T10.TR", "L1.AP", "L1.TR", "L3.AP", "L3.TR")
rownames(df.cont.reg)<-c("five_one_imp", "five_two_imp", "five_three_imp", "five_four_imp", "five_five_imp")
write.csv(df.cont.reg, file="nrmse.continuous.5.RF.csv")
```